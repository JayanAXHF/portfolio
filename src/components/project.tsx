...
const Project = ({ index, title, description, link }: Props) => {
  return (
    <a href={link} className="cursor-none">
      <div
        className={`w-full lg:h-24 relative before:absolute before:top-0 before:h-px before:w-[200vw] transition-[background] duration-600 ease-in-out
      ...
      >
        ...
        <div className="w-full flex flex-row justify-between items-center grow px-5">
          <h2 className="md:text-4xl text-2xl">{title}</h2>
          <p className="text-md max-w-1/2 text-wrap overflow-hidden">
            {description}
          </p>
        </div>
      </div>
